...
function cliLogin() {
	...
	$.ajax({
		url : basePath+'/user/logon.htm',
		type : 'post',
		data : {phoneNum:txtUser,password:txtPwd},
		success : function(result,status){
		      var json = jQuery.parseJSON(result);
		      if(json.code*1==-1){
		    	  alert('用户名或密码错误！');
		      }else{
		    	  //var destUrl = getParameter('goto');
		    	  var destUrl = json.data;
		    	  if(destUrl == null || destUrl == undefined || destUrl == ''){
		    		  destUrl = basePath+"/index/index.html";
		    	  }
...
function Sendpwd(sender,operatetype) {
	var time=120;
	...
	var phoneRandomCode = $.trim($("#phoneRandomCode").val());
	if ($.trim(phoneRandomCode) == "") {
		Tip('请填写随机验证码！');
		$("#phoneRandomCode").focus();
		return;
	}
	
	$.ajax({
		url : basePath+'/user/sendVerifyCode.htm',
		type : 'post',
		data : {phoneNum:phones,operatetype:operatetype, randomCode : phoneRandomCode},
		success : function(result,status){
		      var json = jQuery.parseJSON(result);
		      if(json.code != 0){
		    	  Tip(json.desc);
